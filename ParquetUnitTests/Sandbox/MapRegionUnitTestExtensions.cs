...
namespace ParquetUnitTests.Sandbox
{
    ...
    internal static class MapRegionUnitTestExtensions
    {
        ...
        {
            for (var x = 0; x < All.ParquetsPerRegionDimension; x++)
            {
                for (var y = 0; y < All.ParquetsPerRegionDimension; y++)
                {
                ...
                in_mapRegion.TrySetBlock(TestParquets.TestBlock.ID, new Vector2Int(x, All.ParquetsPerRegionDimension - 1));
            }
            for (var y = 0; y < All.ParquetsPerRegionDimension; y++)
            {
                in_mapRegion.TrySetBlock(TestParquets.TestBlock.ID, new Vector2Int(0, y));
                in_mapRegion.TrySetBlock(TestParquets.TestBlock.ID, new Vector2Int(All.ParquetsPerRegionDimension - 1, y));
            }
