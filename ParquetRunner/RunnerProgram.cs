    ...
    public static class TestModels
    {
        ...
        static TestModels()
        {
            ...
            for (var y = 0; y < TestMapChunk.DimensionsInParquets.Y; y++)
            {
                ...
                {
                    TestMapChunk.TrySetFloorDefinition(TestFloor.ID, new Vector2D(x, y));
                }

                TestMapChunk.TrySetBlockDefinition(TestBlock.ID, new Vector2D(0, y));
                TestMapChunk.TrySetBlockDefinition(TestBlock.ID, new Vector2D(TestMapChunk.DimensionsInParquets.X - 1, y));
            }
            for (var x = 0; x < TestMapChunk.DimensionsInParquets.X; x++)
            {
                TestMapChunk.TrySetBlockDefinition(TestBlock.ID, new Vector2D(x, 0));
                TestMapChunk.TrySetBlockDefinition(TestBlock.ID, new Vector2D(x, TestMapChunk.DimensionsInParquets.Y - 1));
            }
            TestMapChunk.TrySetFurnishingDefinition(TestFurnishing.ID, new Vector2D(1, 2));
            TestMapChunk.TrySetCollectibleDefinition(TestCollectible.ID, new Vector2D(3, 3));
            #endregion
            #endregion
